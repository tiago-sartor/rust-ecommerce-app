use crate::server::handlers::backend::customers::Data;
use crate::utils::{Context, hypertext_elements};
use hypertext::prelude::*;

pub fn admin_customers_template(ctx: &Context<(), Data>) -> impl Renderable {
    let Data { customers, count, page, limit, total_orders, total_ltv } = &ctx.data;
            ...
            <div class="overflow-hidden rounded-xl border border-neutral-200 bg-white dark:border-neutral-800 dark:bg-white/3">
                        ...
                        <div class="flex-col gap-3 sm:flex sm:flex-row sm:items-center">
                            <div class="relative" x-data="{ showFilter: false }">
                                ...
                                <div x-cloak x-show="showFilter" x-on:click.away="showFilter = false" class="absolute flex flex-col gap-1 right-0 z-10 mt-2 w-40 rounded-lg border border-neutral-200 bg-white p-3 shadow-lg dark:border-neutral-700 dark:bg-neutral-800">
                                    ...
                                    <a href=(format!("/admin/emails?filter_by={}", "sent")) class="cursor-pointer text-sm flex items-center gap-3 rounded-lg px-3 py-2 font-medium text-neutral-700 hover:bg-neutral-100 hover:text-neutral-700">
                                        "Sent"
                                    </a>
                                    <a href=(format!("/admin/emails?filter_by={}", "failed")) class="cursor-pointer text-sm flex items-center gap-3 rounded-lg px-3 py-2 font-medium text-neutral-700 hover:bg-neutral-100 hover:text-neutral-700">
                                        "Failed"
                                    </a>
                                </div>
                            </div>
                            ...
                            @for customer in customers {
                                <tr class="transition hover:bg-neutral-50 dark:hover:bg-neutral-900">
                                    <td class="w-14 px-5 py-4 whitespace-nowrap">
                                        <label for=(customer.id) x-on:click.prevent=(format!("toggleSelect({})", customer.id)) class="cursor-pointer text-sm font-medium text-neutral-700 select-none dark:text-neutral-400">
                                            <input id=(customer.id) type="checkbox" class="sr-only" x-bind:checked=(format!("selected.includes({})", customer.id)) >
                                            <span x-bind:class=(format!("selected.includes({}) ? 'border-indigo-500 bg-indigo-500' : 'bg-transparent border-neutral-300 dark:border-neutral-700'", customer.id)) class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]">
                                    ...
                                    <td class="px-5 py-4 whitespace-nowrap">
                                        <div class="flex flex-col gap-1">
                                            <span class="text-sm font-semibold text-neutral-700 dark:text-neutral-400">
                                    ...
                                    <td class="px-5 py-4 whitespace-nowrap">
                                        <div x-data="actionDropdown()"  class="relative flex justify-center">
                                            ...
                                            <div x-cloak x-show="open" x-on:click.outside="open = false" x-ref="dropdown" class="fixed z-10 w-40 rounded-lg border border-neutral-200 bg-white p-2 shadow-lg dark:border-neutral-700 dark:bg-neutral-800">
                                                <form method="GET" action=(format!("/admin/emails/{}/details", customer.id))>
                                                    <button x-on:click.prevent=(format!("fetchDetails({})", customer.id)) class="text-xs flex w-full rounded-lg px-3 py-2 text-left font-medium text-neutral-500 hover:bg-neutral-100 hover:text-neutral-700 dark:text-neutral-400 dark:hover:bg-white/5 dark:hover:text-neutral-300">
                                                        "Details"
                                                    </button>
                                                </form>
                                                <form method="POST" action=(format!("/admin/emails/{}/resend", customer.id))>
                                                    <input type="hidden" name="csrf_token" value=(ctx.csrf_token.0) />
                                                ...
                                                <form method="POST" action=(format!("/admin/emails/{}/delete", customer.id))>
                                                    <input type="hidden" name="csrf_token" value=(ctx.csrf_token.0) />
                ...
                <div
                    ...
                    <div class="flex w-full items-center justify-between gap-2 rounded-lg bg-neutral-50 p-4 sm:w-auto sm:justify-normal sm:rounded-none sm:bg-transparent sm:p-0 dark:bg-neutral-900 dark:sm:bg-transparent">
                        <a
                            x-bind:href=(if *page > 1 { let p = *page - 1; format!("true ? '/admin/emails?page={p}&limit={limit}&filter_by={}' : ''", "") } else { "false".to_string() })
                            class="
                        ...
                        <ul class="hidden items-center gap-0.5 sm:flex">
                            ...
                            <li>
                                @if i == *page {
                                    ...
                                    <a href=(format!("/admin/emails?page={i}&limit={limit}&filter_by={}", "")) class="flex h-10 w-10 items-center justify-center rounded-lg text-sm font-medium hover:bg-indigo-500 text-neutral-700 dark:text-neutral-400 hover:text-white dark:hover:text-white">
                                        <span>(i)</span>
                                    </a>
                                }
                            </li>
                        ...
                        <a
                            x-bind:href=(if *page < total_pages { let p = *page + 1; format!("true ? '/admin/emails?page={p}&limit={limit}&filter_by={}' : ''", "") } else { "false".to_string() })
                            class="
