                        ...
                        <div class="flex-col gap-3 sm:flex sm:flex-row sm:items-center">
                            <div class="relative" x-data="{ showFilter: false }">
                                ...
                                <div x-cloak x-show="showFilter" x-on:click.away="showFilter = false" class="absolute flex flex-col gap-1 right-0 z-10 mt-2 w-40 rounded-lg border border-neutral-200 bg-white p-3 shadow-lg dark:border-neutral-750 dark:bg-neutral-800">
                                    <a href="/admin/emails?filter_by=all" class="cursor-pointer text-sm flex items-center gap-3 rounded-lg px-3 py-2 font-medium text-neutral-750 hover:bg-neutral-100 hover:text-neutral-750">
                                        "All"
                                    </a>
                                    <a href=(format!("/admin/emails?filter_by={}", Status::Sent.to_string().to_lowercase())) class="cursor-pointer text-sm flex items-center gap-3 rounded-lg px-3 py-2 font-medium text-neutral-750 hover:bg-neutral-100 hover:text-neutral-750">
                                        "Sent"
                                    </a>
                                    <a href=(format!("/admin/emails?filter_by={}", Status::Failed.to_string().to_lowercase())) class="cursor-pointer text-sm flex items-center gap-3 rounded-lg px-3 py-2 font-medium text-neutral-750 hover:bg-neutral-100 hover:text-neutral-750">
                                        "Failed"
                                    </a>
                                </div>
                            </div>
                            ...
                            @for log in logs {
                                <tr class="transition hover:bg-neutral-50 dark:hover:bg-neutral-900">
                                    <td class="w-14 px-5 py-4 whitespace-nowrap">
                                        <label for=(log.id) x-on:click.prevent=(format!("toggleSelect({})", log.id)) class="cursor-pointer text-sm font-medium text-neutral-750 select-none dark:text-neutral-400">
                                            <input id=(log.id) type="checkbox" class="sr-only" x-bind:checked=(format!("selected.includes({})", log.id)) >
                                            <span x-bind:class=(format!("selected.includes({}) ? 'border-indigo-500 bg-indigo-500' : 'bg-transparent border-neutral-300 dark:border-neutral-750'", log.id)) class="flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px]">
                                        ...
                                        <div x-data="actionDropdown()"  class="relative flex justify-center">
                                            ...
                                            <div x-cloak x-show="open" x-on:click.outside="open = false" x-ref="dropdown" class="fixed z-10 w-40 rounded-lg border border-neutral-200 bg-white p-2 shadow-lg dark:border-neutral-750 dark:bg-neutral-800">
                                                <form method="GET" action=(format!("/admin/emails/{}/details", log.id))>
                                                    <button x-on:click.prevent=(format!("fetchDetails({})", log.id)) class="text-xs flex w-full rounded-lg px-3 py-2 text-left font-medium text-neutral-500 hover:bg-neutral-100 hover:text-neutral-750 dark:text-neutral-400 dark:hover:bg-white/5 dark:hover:text-neutral-300">
                                                        "Details"
                                                    </button>
                                                </form>
                                                <form method="POST" action=(format!("/admin/emails/{}/resend", log.id))>
                                                    <input type="hidden" name="csrf_token" value=(ctx.csrf_token.0) />
                                                    <button class="text-xs flex w-full rounded-lg px-3 py-2 text-left font-medium text-neutral-500 hover:bg-neutral-100 hover:text-neutral-750 dark:text-neutral-400 dark:hover:bg-white/5 dark:hover:text-neutral-300">
                                                        "Resend"
                                                    </button>
                                                </form>
                        ...
                        <ul class="hidden items-center gap-0.5 sm:flex">
                            ...
                            <li>
                                @if i == *page {
                                    ...
                                    <a href=(format!("/admin/emails?page={i}&limit={limit}&filter_by={filter_by}")) class="flex h-10 w-10 items-center justify-center rounded-lg text-sm font-medium hover:bg-indigo-500 text-neutral-750 dark:text-neutral-400 hover:text-white dark:hover:text-white">
                                        <span>(i)</span>
                                    </a>
                                }
                            </li>
